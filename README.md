...
└── docs/                      # project report (names masked)
```
...
Then open `http://127.0.0.1:5000/login`. `README_project.md` lists the seeded account for each of
the four roles; the role selected in the dropdown must match the account's stored profile or the
login is rejected.
...
- Names and student IDs in `docs/` are masked, so the contribution table and the minutes can be read
  for structure but not for who-was-who beyond my own row.
